...
class OrdersDBWidget(QWidget):
    ...
    def __init__(self, order_entry_factory, refresh_dashboard_callback=None, show_order_entry_callback=None, parent=None):
        ...
        for row_index, order in enumerate(orders):
            ...
            if len(production_items) == 0:
                ...
                production_label = QLabel(
                    ...
                    f"nawój: {item.roll_length}, rdzeń: {item.core}, pak.: {item.packaging}"
                )
                production_label.setFont(QFont("Segoe UI", 10))
                self.table.setCellWidget(row, 6, production_label)
                self.table.setRowHeight(row, 28)
    ...
    def copy_selected_order(self):
        ...
        if self.show_order_entry_callback:
            self.show_order_entry_callback(copy_order=order)
        else:
            order_widget = self.order_entry_factory(copy_order=order)
            order_widget.show()
